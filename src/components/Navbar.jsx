...
const Navbar = () => {
  ...
  // Smooth scroll shrink effect AFTER layout stabilizes
  useEffect(() => {
  ...
  return (
    <header className="fixed top-4 inset-x-0 mx-auto z-50 w-[94%] max-w-6xl">
      <nav
        className={`
          flex items-center justify-between px-6 h-14 rounded-full relative
          overflow-hidden backdrop-blur-xl bg-black/30 border border-gray-500/10
          ...
          md:animate-[float_4s_ease-in-out_infinite]
        `}
      >
        {/* Logo */}
          ...
          {navLinks.map((link) => {
            ...
            return (
              <Link
                ...
                to={link.href}
                className={`
                  text-md relative transition-all duration-300
                  ${
                    isActive
          ...
          {navLinks.map((link) => {
            ...
            return (
              <Link
                ...
                className={`
                  block w-full text-left py-3 border-b border-purple-500/10
